...
The reverse direction needs no separate tagging: `LivePreviewInspectorListener` listens for focus anywhere in the admin form, reads the field's own path straight off its DOM (the `id` every field renders, or Lexical's `data-field-path`) and posts it to the iframe, where `LivePreviewInspectorClient` looks up the tagged element for that path and scrolls/flashes it - no click, no hover state, and it works for any field reachable via `pathOf()`, stega, or value matching. A rich-text field's own path never tags a single element (only the runs inside it do), so the message also carries **where the caret is** — as the same collapsed text window the click direction uses — and the preview scrolls to the element rendering that very paragraph rather than to the first run of the field. Clicking from one paragraph to another inside an already-focused editor fires no focus event, so clicks count as well; typing and arrow keys deliberately don't. Focusing a plain field with no content under its path (a title, a date) scrolls to that field's own tagged element instead. Only *your* focus counts: the focus a reveal itself performs is not sent back, so clicking in the preview never scrolls the preview - each direction moves only the other side.
...
- The preview follows the caret on focus and on clicks inside the editor, not while typing or on arrow keys - so it never scrolls out from under you mid-sentence.
